...
# -*- coding: utf-8 -*-
"""
Исправление конфигурации Nginx для правильной обработки assets
"""
...
def main():
    print("="*60)
    print("ИСПРАВЛЕНИЕ КОНФИГУРАЦИИ NGINX ДЛЯ ASSETS")
    print("="*60)
...
server {
    ...
    # Статические assets - ВАЖНО: этот блок должен быть ПЕРЕД /app
    location /app/assets/ {
    ...
    # Frontend - статические файлы (favicon, etc)
    location /app {
...
"""
        ...
        # Проверяем доступность JS файла
        print("\n4. ПРОВЕРКА JS ФАЙЛА:")
        success, output, error = ssh_exec(ssh, "curl -k -I https://72.56.79.153/app/assets/index-fR8kCqPq-1769869492803.js 2>&1 | head -10")
        print(output)
        
        if "Content-Type: application/javascript" in output or "content-type: application/javascript" in output.lower():
            print("  [OK] MIME тип правильный!")
        elif "200" in output or "HTTP/2 200" in output:
            print("  [OK] Файл доступен")
        else:
            print("  [WARNING] Проверьте заголовки выше")
