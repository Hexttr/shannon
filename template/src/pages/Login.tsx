...
export default function Login() {
  ...
  useEffect(() => {
    if (isAuthenticated) {
      ...
      window.__DEBUG__?.log('[Login] Пользователь авторизован, редирект на:', from);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    setIsLoading(true);
    window.__DEBUG__?.log('[Login] Попытка входа:', username);
    try {
      await login(username, password);
      window.__DEBUG__?.log('[Login] Вход успешен');
      ...
      window.__DEBUG__?.log('[Login] Ошибка входа:', err);
      window.__DEBUG__?.log('[Login] Статус ответа:', err.response?.status);
      window.__DEBUG__?.log('[Login] Данные ответа:', err.response?.data);
      ...
      if (err.response?.status === 422) {
        ...
        const errorMessage = err.response?.data?.error || err.response?.data?.message || err.message || 'Ошибка входа';
                ...
                <div className="flex-1">
                  ...
                  {attemptsRemaining && (
                    <p className="text-xs text-red-300 mt-1">
                      Попробуйте снова через {attemptsRemaining} {attemptsRemaining === 1 ? 'минуту' : 'минут'}
                    </p>
                  )}
                </div>
              </div>
