...
import type { Service, CreateServiceRequest, UpdateServiceRequest, Vulnerability } from '../types';
import {
  FiPlus,
  FiEdit2,
  FiTrash2,
...
export default function Services() {
  ...
  const lastPentestIds = useMemo(() => {
    ...
    services.forEach(service => {
      const servicePentests = pentests.filter(p => p.target_url === service.url);
  ...
  // Загружаем уязвимости для каждого пентеста (максимум 10)
  const vulnerabilityQueries = [
    useQuery({
  ...
  const vulnerabilitiesMap = useMemo(() => {
    ...
    pentestIdsToLoad.forEach((pentestId, index) => {
      const query = vulnerabilityQueries[index];
      if (query?.data) {
    ...
    return map;
  }, [pentestIdsToLoad, vulnerabilityQueries]);
  ...
  const getServiceStats = (serviceUrl: string) => {
    const servicePentests = pentests.filter(p => p.target_url === serviceUrl);
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="text-xs text-gray-500 min-w-0 flex-1">
                          <div className="flex items-center gap-1 mb-1">
                            ...
                            <span className="truncate">Создан: {new Date(service.created_at).toLocaleDateString('ru-RU')}</span>
                          </div>
                          {service.updated_at !== service.created_at && (
                            <div className="text-xs text-gray-600 truncate">
                              Обновлен: {new Date(service.updated_at).toLocaleDateString('ru-RU')}
                            </div>
                          )}
                        </div>
                      </div>
