...
import type { Service, Pentest } from '../types';
import {
  FiServer,
  FiFileText,
  FiDownload,
...
export default function Reports() {
  ...
  // Временно отключаем проверку отчетов, так как методы еще не реализованы
  const reportChecks = useQuery({
    queryKey: ['report-exists', completedPentests.map(p => p.id)],
    queryFn: async () => {
      // Временно возвращаем все false, пока методы не реализованы
      return completedPentests.reduce((acc, pentest) => {
        acc[pentest.id] = false;
        return acc;
  ...
  const handleGenerateReport = async (pentestId: string) => {
    ...
    try {
      ...
      alert('Генерация отчетов пока не реализована');
      await reportChecks.refetch();
    } catch (error: any) {
      console.error('Ошибка при генерации отчета:', error);
      const errorMessage = error.response?.data?.error || error.message || 'Ошибка при генерации отчета';
  ...
  const handleDownloadReport = async (pentestId: string) => {
    ...
    try {
      ...
      alert('Скачивание отчетов пока не реализовано');
                              ...
                              {reportExists ? (
                                <>
                                  <button
                                    ...
                                    className="w-full bg-gradient-to-r from-green-600 to-green-800 hover:from-green-700 hover:to-green-900 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                  >
                                    {isGenerating ? (
                                      <>
                                  ...
                                  <button
                                    onClick={() => handleGenerateReport(pentest.id)}
                                    disabled={isGenerating}
                                    className="w-full bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded-lg text-xs font-medium transition-colors duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                ...
                                <button
                                  onClick={() => handleGenerateReport(pentest.id)}
                                  disabled={isGenerating}
                                  className="w-full bg-gradient-to-r from-blue-600 to-blue-800 hover:from-blue-700 hover:to-blue-900 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                  {isGenerating ? (
                                    <>
                                      <FiLoader className="w-4 h-4 animate-spin" />
