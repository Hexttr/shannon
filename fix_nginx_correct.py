...
# -*- coding: utf-8 -*-
"""
Правильная конфигурация Nginx - убираем try_files из /app
"""
...
server {
    ...
    # Статические assets - rewrite на /assets/
    location ^~ /app/assets/ {
    ...
    # Frontend routes - только для несуществующих файлов
    location /app {
        # Без try_files, чтобы не перехватывать assets
        # Если файл существует - отдаем его, иначе 404
    }
    
    # Fallback для SPA - только если файл не найден
    location ~ ^/app/[^/]+$ {
...
"""
        ...
        # Проверяем JS файл
        print("\n4. ПРОВЕРКА JS ФАЙЛА:")
        success, output, error = ssh_exec(ssh, "curl -k -I https://72.56.79.153/app/assets/index-fR8kCqPq-1769869492803.js 2>&1 | head -10")
        print(output)
        
        # Проверяем содержимое
        success2, output2, error2 = ssh_exec(ssh, "curl -k -s https://72.56.79.153/app/assets/index-fR8kCqPq-1769869492803.js | head -5")
        if "import" in output2 or "function" in output2 or "const" in output2 or "=>" in output2 or "var qN" in output2:
            print("\n  [OK] Файл содержит JavaScript код!")
            print(f"  Первые символы: {output2[:150]}")
        else:
