...
# -*- coding: utf-8 -*-
"""
Исправление Nginx с правильным приоритетом для assets
"""
...
def main():
    print("="*60)
    print("ИСПРАВЛЕНИЕ NGINX С ПРИОРИТЕТОМ ДЛЯ ASSETS")
    print("="*60)
    ...
    try:
        # Используем ^~ для приоритета перед обычными location
        nginx_config = """server {
    listen 80;
...
server {
    ...
    # Статические assets - ^~ дает приоритет перед обычными location
    location ^~ /app/assets/ {
...
"""
        ...
        # Проверяем JS файл
        print("\n4. ПРОВЕРКА JS ФАЙЛА:")
        success, output, error = ssh_exec(ssh, "curl -k -I https://72.56.79.153/app/assets/index-fR8kCqPq-1769869492803.js 2>&1 | grep -i 'content-type'")
        print(f"  Content-Type: {output}")
        ...
        if "import" in output2 or "function" in output2 or "const" in output2 or "!" in output2:
            ...
            print(f"  Первые строки: {output2[:150]}")
        else:
            print(f"  [ERROR] Файл не содержит JS код: {output2[:200]}")
