...
# -*- coding: utf-8 -*-
"""
Исправление Nginx с использованием root вместо alias
"""
...
def main():
    print("="*60)
    print("ИСПРАВЛЕНИЕ NGINX С ROOT ВМЕСТО ALIAS")
    print("="*60)
    ...
    try:
        # Используем root вместо alias - это более надежно
        nginx_config = """server {
    listen 80;
...
server {
    ...
    # Статические assets - используем root для более надежной работы
    location /app/assets/ {
...
"""
        ...
        # Проверяем JS файл
        print("\n4. ПРОВЕРКА JS ФАЙЛА:")
        success, output, error = ssh_exec(ssh, "curl -k -I https://72.56.79.153/app/assets/index-fR8kCqPq-1769869492803.js 2>&1")
        print(output)
        
        if "application/javascript" in output.lower() or "application/x-javascript" in output.lower():
