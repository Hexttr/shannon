...
def main():
    print("="*60)
    print("ФИНАЛЬНОЕ ИСПРАВЛЕНИЕ NGINX")
    print("="*60)
    ...
    try:
        # Правильная конфигурация - assets ПЕРЕД общим /app
        nginx_config = """server {
    listen 80;
...
server {
    ...
    # Статические assets - ВАЖНО: должен быть ПЕРЕД /app
    location ~ ^/app/assets/(.*)$ {
        alias /root/shannon/template/dist/assets/$1;
        expires 1y;
...
"""
        ...
        if "application/javascript" in output.lower() or "application/x-javascript" in output.lower():
            ...
            if "import" in output2 or "function" in output2 or "const" in output2:
                print("  [OK] Файл содержит JavaScript код")
            else:
                print(f"  [WARNING] Содержимое: {output2[:100]}")
