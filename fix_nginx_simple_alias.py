...
# -*- coding: utf-8 -*-
"""
Простая конфигурация Nginx с правильным alias
"""
...
def main():
    print("="*60)
    print("ПРОСТАЯ КОНФИГУРАЦИЯ NGINX")
    print("="*60)
    ...
    try:
        # Самая простая конфигурация: используем root для всего
        nginx_config = """server {
    listen 80;
...
server {
    ...
    # Root для всех статических файлов
    root /root/shannon/template/dist;
    
    # Статические assets - используем rewrite для правильного пути
    location ~ ^/app/assets/(.+)$ {
        # rewrite перенаправляет на /assets/... относительно root
        rewrite ^/app/assets/(.+)$ /assets/$1 break;
    }
    
    # Frontend - все остальное
    location /app {
...
"""
        ...
        if "import" in output2 or "function" in output2 or "const" in output2 or "=>" in output2 or "!" in output2:
            ...
            print(f"  Первые символы: {output2[:100]}")
        else:
